resource "aws_iam_instance_profile" "cd4ml_eks_node" {
  name = "cd4ml_eks_node-${var.cd4ml_env}"
  role = var.eks_nodes_role_name
}

resource "aws_launch_configuration" "eks_node_launch" {
  associate_public_ip_address = true
  iam_instance_profile        = aws_iam_instance_profile.cd4ml_eks_node.name
  ...
  depends_on = [aws_iam_instance_profile.cd4ml_eks_node]
}
...
resource "aws_autoscaling_group" "eks_worker_autoscaling" {
  ...
  name                 = "eks_worker_autoscaling-${var.cd4ml_env}"
  vpc_zone_identifier  = flatten("${var.Public_Subnet_id_list}")

  tag {
    key                 = "Name"
    value               = "eks_worker_autoscaling-${var.cd4ml_env}"
